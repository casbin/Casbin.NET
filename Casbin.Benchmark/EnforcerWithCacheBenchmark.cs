...
namespace Casbin.Benchmark
{
    ...
    public class EnforcerWithCacheBenchmark
    {
        ...
        private string NowTestUserNameLowerBound { get; set; }
        private string NowTestDataNameLowerBound { get; set; }
        private string NowTestUserNameUpperBound { get; set; }
        private string NowTestDataNameUpperBound { get; set; }
        ...
        [GlobalSetup(Targets = new[] { nameof(RbacModelWithLargeScale), nameof(RbacModelWithLargeScaleLowerBound), nameof(RbacModelWithLargeScaleUpperBound) })]
        ...
        private void GlobalSetupForRbacModelWithScale(int groupCount, int userCount)
        {
            ...
            {
                //Empirical data: each group will be 15 chars long. Each asset
                //will be a 20 char long string (if think file path, this is a
                //short path)
                var groupExt = $"group{i.ToString().PadLeft(10, '0')}";
                var dataExt = $"data{(i / 10).ToString().PadLeft(16, '0')}";
                //policyList.Add(new[] { groupExt, dataExt, "read" }.ToList());
                policyList.Add(new[] { groupExt, dataExt, "write" }.ToList());
                policyList.Add(new[] { groupExt, dataExt, "grant" }.ToList());
            }
            NowTestDataNameLowerBound = policyList[10][0];
            NowTestDataNameUpperBound = policyList[groupCount-10][0];
            NowEnforcer.AddPolicies(policyList);

            policyList.Clear();
            for (int i = 0; i < userCount; i++)
            {
                //Given that a user is usually referred by it's email, which in
                //avg is 25 char long, lets take this worst case cenario
                policyList.Add(new[] { $"user{i.ToString().PadLeft(21, '0')}", $"group{(i / 10).ToString().PadLeft(10, '0')}" }.ToList());
            }
            NowTestUserNameLowerBound = policyList[10][0];
            NowTestUserNameUpperBound = policyList[userCount-50][0];
            NowEnforcer.EnableAutoBuildRoleLinks(false);
            NowEnforcer.AddGroupingPolicies(policyList);
            NowEnforcer.BuildRoleLinks();

            NowTestUserName = $"user{(userCount / 2 + 1).ToString().PadLeft(21, '0')}"; // if 1000 => 501...
            NowTestDataName = $"data{(groupCount / 10 - 1).ToString().PadLeft(16, '0')}"; // if 100 => 9...
        ...
        //[Benchmark(Description = "RBAC (large), 110000 rules (100000 users, 10000 roles)")]
        [BenchmarkCategory("RbacModel")]
        public void RbacModelWithLargeScaleLowerBound()
        {
            _ = NowEnforcer.Enforce(NowTestUserNameLowerBound, NowTestDataNameLowerBound, "read");
        }

        [Benchmark]
        //[Benchmark(Description = "RBAC (large), 110000 rules (100000 users, 10000 roles)")]
        [BenchmarkCategory("RbacModel")]
        public void RbacModelWithLargeScaleUpperBound()
        {
            _ = NowEnforcer.Enforce(NowTestUserNameUpperBound, NowTestDataNameUpperBound, "read");
        }
